document.querySelector('title').appendChild(document.createTextNode(browser.runtime.getManifest().name + ' - Options'));
document.addEventListener('DOMContentLoaded', function() {

    // on load populate options from storage
    document.querySelectorAll('.option').forEach(function(element) {
        var name = element.getAttribute('name');
        browser.storage.sync.get(name, function(obj) {
            if (obj[name]) {
    ...
    document.getElementById('sound').addEventListener('change', function(event) {
        browser.storage.sync.set({'sound_option': this.value});
    ...
    function create_label_row_inputs() {
        ...
        btn.addEventListener('click', function(event) {
            ...
            // save
            browser.storage.sync.get('labels', function(obj){
                if (!obj.labels) {
                    obj.labels = {};
                }
    ...
    function get_label_row(values, id) {
        ...
        delete_link.addEventListener('click', function(event) {
            ...
            browser.storage.sync.get('labels', function(obj){
                ...
                browser.storage.sync.get('time_logs', function(object) {
    ...
    document.getElementById('time_limit').addEventListener('change', function(event) {
        browser.storage.sync.set({'time_limit_option': parseFloat(this.value)});
    });

    document.getElementById('idle').addEventListener('change', function(event) {
        browser.storage.sync.set({'idle': parseFloat(this.value)});
    });
