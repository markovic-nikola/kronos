document.addEventListener("DOMContentLoaded", function () {
	...
	$stopBtn.setAttribute(
		"title",
		manifest.commands.stop.suggested_key.default
	);
	...
	$playPauseBtn.setAttribute(
		"title",
		manifest.commands.play_pause.suggested_key.default
	);
	...
	port.onMessage.addListener(function (msg) {
		if (msg.time) {
			...
			document
				...
				.appendChild(document.createTextNode(msg.time));
		}

		if (msg.isRunning !== null && typeof msg.isRunning !== "undefined") {
			document
				.getElementById("play_pause")
				.setAttribute("name", msg.isRunning ? "pause" : "play"); // toggle play pause icon/btn
		}
		...
		if (msg.timeRaw !== null && typeof msg.timeRaw !== "undefined") {
			if (msg.timeRaw > 0) {
				...
				document.getElementById("save_time_input").style.display =
					"none";
			}
		...
		if (
			msg.timerLimitReached !== null &&
			typeof msg.timerLimitReached !== "undefined"
		) {
	...
	function showUpdateLink(version) {
		...
		a.href =
			"https://markovic-nikola.github.io/kronos/#v" +
			version.replace(".", "");
	...
	chrome.storage.sync.get("labels", function (obj) {
		...
		if (!obj.labels || Object.keys(obj.labels).length == 0) {
			...
			options_link.appendChild(
				document.createTextNode("Empty! Add labels.")
			);
			options_link.href = "#";
			...
			for (key in obj.labels) {
				...
				label.appendChild(
					document.createTextNode(obj.labels[key].name)
				);
	...
	// save time log
	document
		.getElementById("save_time_btn")
		.addEventListener("click", function (event) {
			var id = new Date().valueOf();
			var $name = this.parentNode.querySelector(
				'input[name="save_time_input"]'
			);
			var name = $name.value;
			var label = this.parentNode.querySelector(
				'input[name="label_color"]:checked'
			)
			...
			if (!label) {
				$error_msg.appendChild(
					document.createTextNode("Label is not selected.")
				);
			} else if (!name) {
				$error_msg.appendChild(
					document.createTextNode("Name is empty.")
				);
				return;
			} else if (name.length > 100) {
				$error_msg.appendChild(
					document.createTextNode(
						"Name is too long, maximum characters: 100."
					)
				);
				return;
			} else {
