document
	.querySelector("title")
	.appendChild(
		document.createTextNode(chrome.runtime.getManifest().name + " - Options"),
	);
document.addEventListener("DOMContentLoaded", function () {
	...
	function create_label_row_inputs() {
		...
		inputRate.setAttribute("placeholder", "Hourly rate");
		inputRate.setAttribute("class", "ml15");
		inputRate.setAttribute("min", "0");
		inputRate.setAttribute("step", "0.01");
		row.appendChild(inputRate);
		...
		btn.addEventListener("click", function (event) {
			...
			if (!values.name) {
				row
					...
					.appendChild(document.createTextNode("Empty name"));
				return;
			}
			if (!values.color || !Object.values(colors).includes(values.color)) {
				row
					.querySelector(".text-danger")
					.appendChild(document.createTextNode("Invalid color"));
				return;
			}
			if (!values.rate) {
				row
					.querySelector(".text-danger")
					.appendChild(document.createTextNode("Empty rate"));
				return;
			}
			// save
	...
	function get_label_row(values, id) {
		...
		if (values.rate) {
			...
			rate.appendChild(document.createTextNode(` (Rate/hr: ${values.rate})`));
